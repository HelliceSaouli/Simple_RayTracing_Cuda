...
Shader::Shader(const std::string& filePath)
{
	...
	this->m_uniforms[0] = glGetUniformLocation(this->m_program,"transform");
...
{
	// we are in ray tracer so i don't need this
/*
	glm::mat4 transform = cam.getViewProjection() * trans.getModel(); // i should call it mvp

	glm::vec3 posCam   = cam.getPosition();
	glUniformMatrix4fv(this->m_uniforms[0],1,GL_FALSE,&transform[0][0]);
